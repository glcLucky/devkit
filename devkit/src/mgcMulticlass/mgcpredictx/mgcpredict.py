...
def mgcPlotKpibyClass(**kwargs):
    ...
    arr_x_mean = arr_x.reshape(-1, 24, 32).mean(axis = 1)
...
class mgcPredict(object):
    ...
    def __init__(self, **kwargs):
        ...
        # Image size
        self.nb_hours = 24
        self.nb_kpis = len(self.kpi_list)        
    ...
    def mgcLoadNormData(self, **kwargs):
        ...
        self.feat_norm = kwargs['feat_norm_train']
        self.dataset_ref = kwargs['dataset_ref']
        self.df_raw = kwargs['df_raw']

        df_raw_x_w = pd.DataFrame(self.dataset_ref.groupby(['cell_date', 'hour']).sum().unstack('hour'))  
        df_raw_x_w = df_raw_x_w.reindex(index=self.dataset_ref['cell_date'].unique())
        ...
        df_raw_x_w.set_index('cell_date', inplace=True)
        df_raw_x_w.drop(columns = ['index'], inplace = True)
        
        feat_dataset = np.array(self.dataset_ref[self.kpi_list]) # self.feat_norm
    ...
    def mgcViz(self, **kwargs):
        ...
        print('')
        print('total number of {0: .0f} unique cell, day'.format(len(self.df_raw['cell_date'].unique())))
