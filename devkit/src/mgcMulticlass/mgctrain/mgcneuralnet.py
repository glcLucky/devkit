...
# the first report_to_df is NOT used, to be deleted.
def report_to_df(report):
    ...
    """
    col_names = ["Classes", "precision", "recall", "f1-score", "support"]
    #print("original \n")
    print(report)
    report = re.sub(r" +", " ", report).replace("avg / total", "avg/total").replace("\n ", "\n")
    #print(report)
    report_df = pd.read_csv(StringIO("Classes" + report), sep=' ', index_col=0, names = col_names, encoding='utf-8')
    print(report_df)
    return(report_df)

# this function report_to_df is used in the rest of the codes
def report_to_df(report):
    """
    Transform classifcation report text strings to dataframe
    Arguments:
